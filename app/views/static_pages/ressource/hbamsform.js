 ...
 * 2014-11-5
 * 用于表单验证
 * 只要在需要验证的控件上添加hbamsform属性即可，多个属性用[;]连接
 * 属性列表：
 *      null
 *      email
 ...
 *      less:xxx								小于某个对象的值，冒号后是jq选择器语法
 *      more:xxx								大于某个对象的值，冒号后是jq选择器语法
 *      ajax:fun()
 *      real-time                               实时检查
 *      date                    2014-10-31
 *      time                    10:30:00
 ...
 * 5. 2014-11-20 当网页载入时有隐藏的控件，之后控件显示出来后，其关联的hbamstip不能正确显示位置
 * 6. 2014-11-21 目前不支持属性继承
 * 7. 2014-11-21 实时检查的时候，弹出的hbamstip有时候会导致弹出信息的消息出错
 *
 *
 * ------ change list -------------------------------------------------
...
(function ($, window, document, undefined)
{
    /*
     构造函数
     **/
    var _hbamsform = function (ele, opt)
    {
        this.form = ele;
    ...
    _hbamsform.prototype = { 
        ...
        _load: function ()
        {
            ...
            this.form.find("input:visible").each(function (index, input)
            {
                ...
                {
                    var checker = $(input).hbamsinput({hbamstip: ev.hbamstip});
        ...
        valid:function(){
             ...
             for (index in this.inputs)
             {
                 ...
                 if(this.inputs[index].is_error) return false;
             }
             return true;
        }, 
        /*
    ...
    //添加到jquery
    $.fn.hbamsform = function (options)
    {
        var validation = new _hbamsform(this, options); 
...
{
    //单个input的检查器构造函数
    var _hbamsinput = function (input, opt)
    {
        this.input = input;
        this.rules = [];
    ...
    _hbamsinput.prototype = {
        ...
        init: function ()
        {
            ...
            //是否实时检查
            var hbamsinput = this;
            var rule = this.input.attr("hbamsform");
            if (!!rule && -1 != rule.indexOf("real-time"))
            {
                this.input.blur(function ()
                {
                    hbamsinput.validation();
                });
            }
        ...
        //初始化hbamstip
        _init_hbamstip: function ()
        {
            if (!!this.options.hbamstip)
            {
                var tipoptions = $(this.input).attr("hbamstip");
                ...
                this.options.hbamstip = $(this.input).hbamstip(options);
            }
        },
        ...
        validation: function ()
        {
            ...
            this.rule = this.input.attr("hbamsform");
        ...
        _error: function (rule)
        {
            ...
            if (false == this.is_error)
            {
                ...
                if (!!this.options.hbamstip)
                {
                    this.options.hbamstip.show(msg);
                }
        ...
        {
            this.counter += 1;
            //alert(this.options.hbamstip.id);
         ...
         * */
        judge: {
        	"required" :function(ei, v, p){
        		 if (v.replace(/(^s*)|(s*$)/g, "").length ==0)
                     return ei._error("required");
                 else
                     return ei._success_rule("required");
        	},
            ...
            "less": function (ei, v, p)
            {
                var pair = $(p);
                var result = false;
                if(parseFloat(v)&&parseFloat(pair.val())){
                	result = parseFloat(v)>parseFloat(pair.val());
                }else{
                	result = pair.val() > v ;
                }
                if (result)
                    return ei._error("less");
                else
                    return ei._success_rule("less");
            },
            "more": function (ei, v, p)
            {
                var pair = $(p);
                var result = false;
                if(parseFloat(v)&&parseFloat(pair.val())){
                ...
                if (result)
                    return ei._error("more");
                else
                    return ei._success_rule("more");
            },
            "ajax": function (ei, v, p)
            {
                // 为ajax处理注册自定义事件
                // HTML中执行相关的AJAX时，需要发送事件 hbamsinput-ajax 来通知 hbamsinput
                // 该事件只有一个bool参数，hbamsinput 会根据这个值判断ajax验证是否成功
                ei.input.delegate("", "hbamsinput-ajax", function (e, p)
                {
                    ei.input.unbind("hbamsinput-ajax");
    ...
    $.fn.hbamsinput = function (options)
    {
        var check = new _hbamsinput(this, options);
...
(function ($, window, document, undefined)
{
    ...
    var _hbamstip = function (ele, opt)
    {
        this.parent = ele;
        this.defaults = {
            left: 0, top: 0,
        ...
        this.id = "hbamstip-div-main" + ele[0].id;
    };
    ...
    _hbamstip.prototype = {
        ...
        init: function ()
        {
            ...
            if (tip.length == 0)
            {
                $(document.body).append("<div id=\"" + this.id + "\"><div class=\"hbamstip-text\"></div></div>");

                tip = $("#" + this.id);
                var text = $("#" + this.id + " .hbamstip-text");
                ...
                tip.append("<div class=\"hbamstip-arrow\"></div>");
                var arrow = $("#" + this.id + " .hbamstip-arrow");
                arrow.css({
                    "padding": "0",
                    "margin": "0",
                    "width": "0",
    ...
    $.fn.hbamstip = function (options)
    {
        var tip = new _hbamstip(this, options);
